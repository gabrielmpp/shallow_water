...
from plotlib import plot_task_D, plot_analytical, plot_energy, plot_task_F, plot_task_G
from model import shallow_water



def init_C_grid(L, dx, dy, time_vector):
    ...
    u_coords = {'x': np.arange(-0.5*dx, L + 0.5*dx, dx), 'y': np.arange(0, L, dy)}
    v_coords = {'x': np.arange(0, L, dx), 'y': np.arange(-0.5*dy, L + 0.5*dy, dy)}
    eta_coords = {'x': np.arange(0, L, dx), 'y': np.arange(0, L, dy)}
    nx = config.nx
    ny = config.ny
    ...
    eta_array_dump = xr.DataArray(np.zeros([ny, nx, ntime]), dims = ('y','x','time'), coords = {'y': eta_coords['y'], 'x': eta_coords['x'], 'time': time_vector})
    u_array_dump = xr.DataArray(np.zeros([ny, nx + 1, ntime]), dims = ('y','x','time'), coords = {'y': u_coords['y'], 'x': u_coords['x'],'time': time_vector})
    v_array_dump = xr.DataArray(np.zeros([ny + 1, nx, ntime]), dims = ('y','x','time'), coords = {'y': v_coords['y'], 'x': v_coords['x'],'time': time_vector})
...
def run_integration(sw_model):
    ...
    eta, u, v, eta_dump, u_dump, v_dump = init_C_grid(L, dx, dy, time_vector)
    tau_x, tau_y = sw_model.calc_tau(u.y, L)
    ...
    for t in np.arange(0,ntime-2,2): # I found that copying the equations in reversed order is faster than using an if statement
    ...
    plot_task_D(u, v, eta)  # Reproduce task D plots for the last time step available. Set integration length to 1 in the config file to obtain task D outputs.
...
def run_analytical_solution(sw_model):
    ...
    x_coords = np.arange(0, L, dx)
    y_coords = np.arange(0, L, dy)
    ...
    u_sol = xr.apply_ufunc(lambda u, v, eta: sw_model.analytical_solution(u, v, eta)[0], u_array.x, v_array.y, eta_array)
    v_sol = xr.apply_ufunc(lambda u, v, eta: sw_model.analytical_solution(u, v, eta)[1], u_array.x, v_array.y, eta_array)
    eta_sol = xr.apply_ufunc(lambda u, v, eta: sw_model.analytical_solution(u, v, eta)[2], u_array.x, v_array.y, eta_array)
...
if __name__ == '__main__':
    ...
    sw_model = shallow_water(config.L) # Create instance of sw model with default parameters
    u_s, v_s, eta_s = run_analytical_solution(sw_model)
    eta, u, v = run_integration(sw_model)
    u_error = u.interp(x=u_s.x,y=u_s.y)-u_s
    v_error = v.interp(x=v_s.x,y=v_s.y)-v_s
    eta_error = eta - eta_s
    E_error = calc_energy(u_error, v_error, eta_error, sw_model)
    E_error.name = 'Energy error {dx}km {method}'.format(dx = config.dx/1000, method = config.method)
    E_error.to_netcdf('outputs/energy_error_{method}_resolution_{dx}km.nc'.format(method=config.method, dx = config.dx/1000))
    plot_energy()
    plot_task_F()
    plot_task_G(eta_s)
